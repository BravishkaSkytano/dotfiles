...
while true; do
upd="$(xbps-install -SunM | wc -l)"
list="$(xbps-install -nuM | awk '{print $1}')"
#inst="$(pidof yad)"
...
if (( "$upd" == 0 ));
then
    cat ~/.config/polybar/updateIcons/noupdate.png > ~/.config/polybar/updateIcons/trayIcon.png
    yad --text "$upd Updates" --notification --image="$HOME"/.config/polybar/updateIcons/trayIcon.png
elif (( "$upd" <= 5 ));
then
    cat ~/.config/polybar/updateIcons/modupdate.png > ~/.config/polybar/updateIcons/trayIcon.png
    yad --text "$upd Updates: Right click to see all updates, Left click to perform update" --notification\
        --image="$HOME"/.config/polybar/updateIcons/trayIcon.png --menu="$list" --command "alacritty -e sudo xbps-install -Su"
elif (( "$upd" > 5 ));
then
    cat ~/.config/polybar/updateIcons/lotsupdate.png > ~/.config/polybar/updateIcons/trayIcon.png
    yad --text "$upd Updates: Right click to see all updates, Left click to perform update" --notification\
        --image="$HOME"/.config/polybar/updateIcons/trayIcon.png --menu="$list" --command "alacritty -e sudo xbps-install -Su"
fi
#elif [[ $inst -gt "1" ]]
